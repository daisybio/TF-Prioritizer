...
def check_samplesheet(file_in, file_out):
    ...
    with open(file_in, "r", encoding="utf-8-sig") as fin:
        ## Check header
        MIN_COLS = 3
        HEADER = ["sample", "fastq_1", "fastq_2", "strandedness", "group", "batch"]
        ...
        for line in fin:
            if line.strip():
                ...
                sample, fastq_1, fastq_2, strandedness, group, batch = lspl[: len(HEADER)]
                ...
                if group.find(" ") != -1:
                    print(f"WARNING: Spaces have been replaced by underscores for group: {group}")
                    group = group.replace(" ", "_")
                if not group:
                    print_error("Group entry has not been specified!", "Line", line)
                
                if batch.find(" ") != -1:
                    print(f"WARNING: Spaces have been replaced by underscores for batch: {batch}")
                    batch = batch.replace(" ", "_")
                # Batch is optional, so no error if not specified
