...
count.table           <- read.table(file=opt$count_file,header=TRUE, row.names=NULL, comment.char ="#", sep="\t")
...
print("Preparing data for DESeq2...")
...
write.table(pca.vals,
    file = paste(opt$outprefix, ".pca.vals.txt", sep = ""),
...
write.table(cbind(sample = rownames(sampleDistMatrix), sampleDistMatrix),
    file = paste(opt$outprefix, ".sample.dists.txt", sep = ""),
