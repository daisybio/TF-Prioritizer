...
group_df <- data.frame(row.names = rownames(count_df))

# Loop over each unique group
for (g in unique(metadata_df$group)) {
    # Subset the count_df to the samples in the current group
    # Attention: Requires count_df and metadata_df to have the same order
    group_counts <- count_df[, metadata_df$group == g]
