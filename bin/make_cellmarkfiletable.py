...
parser.add_argument("--markers", help = "path to file with possible markers (default markers are always set)", required = False, type = str)
...
markers = {"CTCF", "ELF5", "GR", "H3K27ac", "H3K27me3", "H3K4me1", "H3K4me3", "MED1", "NFIB", "RNAPolII", "STAT5", "CONTROL"}
...
stages = [os.path.join(input_dir, element) for element in os.listdir(input_dir) if os.path.isdir(os.path.join(input_dir, element)) or os.path.islink(os.path.join(input_dir, element))]

output_list = []
for stage in stages:
    print(stage)
    print(os.listdir(os.path.realpath(stage, strict=False)))
    ...
    for file in os.listdir(stage):
		...
		# Add "_reformatted" tag as the binarize_bam module will run on the reformatted files
        output_list.append((os.path.basename(stage), marker, file[0:-4] + "_reformatted.bam",  control_file[0:-4] + "_reformatted.bam"))     
