...
def check_tfs(tfs, ensg_counts, total_sample_number, gene_symbol_lengths, ensg_sym_map, cutoff, outfile_1):
    ...
    for tf in tfs:
        ...
        if (':' in tf):
            ...
            for s in split_tf_2:
                ...
                if (ensg_length is None or ensg_count is None or ensg_length == 0):
                    ...
                    if (tpm > cutoff):
                        wanna_print = True
                    tpm_values_line = tf_ensg + "\t" + s + "\t" + str(tpm) + "\t" + str(rpk) + "\t" + str(
                        ensg_length) + "\n"
        ...
        if (tpm >= cutoff):
            ...
            should_write.append(0)
        tpm_values_line = tf_ensg + "\t" + split_tf[0] + "\t" + str(tpm) + "\t" + str(rpk) + "\t" + str(
            ensg_length) + "\n"
        output_tpm_values.write(tpm_values_line)
...
def main():
    ...
    if (sys.argv[2] != "NOT_SET" and sys.argv[2] != ""):
        ...
        valid_columns = check_tfs(tfs_splitted, ensg_counts, total_sample_number, gene_symbol_lengths, ensg_sym_map,
                                  float(sys.argv[3]), outfile_1)
