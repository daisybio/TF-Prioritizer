...
if [ -d "$HOME/.igv" ]; then
  ...
  wget https://data.broadinstitute.org/igv/projects/downloads/2.12/IGV_2.12.2.zip
  if ! $docker ; then
    unzip IGV_2.12.2.zip -d "$HOME/.igv"
    igv_dir=$HOME/igv
  else
    unzip IGV_2.12.2.zip -d /srv/dependencies
    mv /srv/dependencies/IGV_2.12.2 /srv/dependencies/igv
