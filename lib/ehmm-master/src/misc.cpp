...
inline void writeVectors_numeric(Rcpp::NumericVector bigv, Rcpp::List vlist, int nthreads){
  int nv = vlist.length();
  std::vector<int> breaks(nv + 1);
  for (int i = 0, acc = 0; i < nv; ++i){
	acc += Rcpp::as<Rcpp::NumericVector>(vlist[i]).length();
	breaks[i+1] = acc;
  }
...
#pragma omp parallel for num_threads(nthreads)
  for (int i = 0; i < nv; ++i){
	...
	Rcpp::NumericVector v = vlist[i];
	memcpy(bigv.begin() + breaks[i], v.begin(), sizeof(double)*v.length());
  }
}
...
// [[Rcpp::export]]
Rcpp::NumericMatrix bindCols_numeric(Rcpp::List vlist, int nthreads=1){
  int ncol = vlist.length();
  int nrow;
  {
	Rcpp::NumericVector v = vlist[0];
	nrow = v.length();
  }
  ...
  writeVectors_numeric(mat, vlist, nthreads);
...
void writeCountsTXT_double(Rcpp::NumericMatrix counts, std::vector<std::string> marks, std::string path){
    ...
    //write counts
    for (int col = 0; col < ncol; ++col){
        Rcpp::MatrixColumn<REALSXP> C = counts.column(col);
