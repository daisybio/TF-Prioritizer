...
test_that("multiple datasets works",{
    ...
    for (method in c("TMM", "RLE")){
        normalizecounts(clist, ehmm:::linearNormalization, method=method)
    }
...
test_that("kfoots error handler", {
    ...
    uflow <- get(load(system.file("extdata/uflow_minimal.Rdata", package="ehmm")))
...
test_that("avgStateProfile", {
	genes <- readRegions(system.file("extdata/genes.bed", package="ehmm"))
	segms <- readRegions(system.file("extdata/5s_segmentation.bed", package="ehmm"))
