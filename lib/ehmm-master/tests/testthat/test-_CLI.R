...
test_that("Command line interface",{
    ...
    truns(
    "ehmm.R getcounts --target ${outdir}/countmat.txt \\
    ...
    truns(
    "ehmm.R getcounts -r ${indir}/contigs.bed \\
    ...
    truns(
    "ehmm.R getcounts --target ${outdir}/countmat_onemark.txt \\
    --regions ${indir}/contigs.bed \\
    --mark H3K4me3:${indir}/H3K4me3.bam")

    truns(
    "ehmm.R segment -c ${outdir}/countmat_onemark.txt -r \\
    ...
    #a more normal usage
    truns(
    "ehmm.R segment -c ${outdir}/countmat.txt -r \\
    ${indir}/contigs.bed -n 10 --nthreads 4 \\
    -a genes:${indir}/genes.bed --maxiter 20  --outdir ${outdir}")
    
    #segment with --model option
    truns(
    "ehmm.R segment -n 10 -m ${outdir}/model.txt \\
    ...
    truns(
    "ehmm.R segment -n 10 -m ${outdir}/incomplete_model.txt \\
    -c ${outdir}/countmat.txt -r ${indir}/contigs.bed --outdir ${outdir}")
    
    #let's try out the notrain, collapseInitP and save_rdata flags
    truns(
    "ehmm.R segment --collapseInitP T --save_rdata \\
    ...
    truns(
    "ehmm.R segment --notrain -m ${outdir}/model.txt -c ${outdir}/countmat.txt \\
    -r ${indir}/contigs.bed -n 10 --outdir ${outdir}")
    ...
    #REPORT
    truns(
    "ehmm.R report -m ${outdir}/model.txt -s ${outdir}/segmentation.bed \\
    --outdir ${outdir}")
    
    #let's try the colors option
    truns(
    "ehmm.R report --colors ${outdir}/colors.txt \\
    ...
    truns(
    "ehmm.R report --labels ${outdir}/labels.txt \\
    -m ${outdir}/model.txt -s ${outdir}/segmentation.bed --outdir ${outdir}")
    ...
    truns(
    "ehmm.R report --labels ${outdir}/labels.txt  -o ${outdir} -m ${outdir}/model.txt \\
...
test_that("multiple datasets",{
    ...
    for (i in seq_along(os)){
        ...
        truns(paste("ehmm.R getcounts -r ${indir}/contigs.bed -t ", targets[i], mline))
    }
    
    #run normalize counts
    cline <- paste0(collapse=" ", "-c ", targets)
    truns(paste0("ehmm.R normalizecounts ", cline))
    ...
    truns(paste0("ehmm.R normalizecounts ", cline, " -s ", triggerOverwrite))
    ...
    truns(paste0("ehmm.R segment ", goodcline, " -n 5 -r ${indir}/contigs.bed -o ${outdir}"))
    
    #check that without labels it fails!
    badcline <- paste0(collapse=" ", "-c ", newtargets)
    tfails(paste0("ehmm.R segment ", badcline, " -n 5 -r ${indir}/contigs.bed -o ${outdir}"))
...
test_that("usage examples/manual", {
    ...
    rmd <- system.file("examples", "cliExamples.Rmd", package="ehmm")
    library(knitr)
