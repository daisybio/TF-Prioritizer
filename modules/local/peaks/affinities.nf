...
process AFFINITY_SUMS {
    ...
    output:
        tuple val(hm), val(group1), val(group2), path("${hm}_${group1}:${group2}_affinitySums.txt")

    script:
    """
    affinity_sums.R --input1 ${affinities1} --input2 ${affinities2} --output ${hm}_${group1}:${group2}_affinitySums.txt
    """
}