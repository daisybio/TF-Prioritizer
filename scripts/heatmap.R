  ...
  for (hm in hms) {
    ...
    for (gene in genes) {
      ...
      for (group_pairing in group_pairings) {
        ...
        co_genes_file <- paste(target_genes_path, co_group, hm, gene, paste(gene, "csv", sep = "."), sep = "/")
        if (!file.exists(co_genes_file)) next

        co_selected_genes <- read.csv(co_genes_file, sep = "\t", nrows = number_of_genes_to_select)
        co_selected_genes <- read.csv(paste(target_genes_path, co_group, hm, gene, paste(gene, "csv", sep = "."), sep = "/"),
                                      sep =
                                        "\t", nrows = number_of_genes_to_select)
