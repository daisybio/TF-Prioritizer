...
public class COM2POSE_lib
{
    ...
    public void run_igv_own_data() throws Exception {
        logger.logLine("[IGV] start taking screenshots for top TFs and their corresponding top target genes (OWN TF DATA)");
        ...
        */
        ...
        for(File f_target_genes_tp : f_root_target_genes.listFiles())
        {
            if(f_target_genes_tp.isDirectory())
            {
                for(File f_target_genes_tp_hm : f_target_genes_tp.listFiles())
                {
                    ...
                    {
                        HashMap<String,ArrayList<String>> tf_target_genes = new HashMap<>();
                        hm_timepoint_tf_target_genes.get(f_target_genes_tp_hm.getName()).put(f_target_genes_tp.getName(),tf_target_genes);
                        ...
                        for(File f_target_genes_tp_hm_tf : f_target_genes_tp_hm.listFiles()) {
                            if (f_target_genes_tp_hm_tf.isDirectory()) {
                                ...
                                BufferedReader br = new BufferedReader(new FileReader(new File(f_target_genes_tp_hm_tf.getAbsolutePath() + File.separator + tf_name + ".csv")));
        ...
        File f_important_tfs_per_hm = new File(options_intern.com2pose_working_directory+File.separator+options_intern.folder_out_distribution+File.separator+options_intern.folder_out_distribution_stats+File.separator+options_intern.folder_out_distribution_stats_HM);
        ...
        for(File fileDir_hm : f_important_tfs_per_hm.listFiles())
        {
            if(fileDir_hm.isDirectory())
            {
                ...
                BufferedReader br = new BufferedReader(new FileReader(new File(fileDir_hm.getAbsolutePath()+File.separator+options_intern.file_suffix_distribution_analysis_plot_stats)));
                String line = br.readLine();
                line= br.readLine();
                while((line= br.readLine())!=null)
                {
                ...
                for(String key_hm: hm_timepoint_tf_target_genes.keySet())
                {
                    ...
                    for(String key_tp : hm.keySet())
                    {
                        ...
                        if(hm_tp.containsKey(tf))
                        {
                            ...
                            //save sessions
                            File f_session_tp_name = new File(f_out_session_tp.getAbsolutePath()+File.separator+options_intern.file_suffix_session);
                            ...
                            File f_input_regions_to_target_genes = new File(options_intern.com2pose_working_directory+File.separator+options_intern.folder_name_tepic_output_raw+File.separator+key_tp+File.separator+key_hm);
    ...
    public void run_igv_chr_wide_data() throws IOException {
        ...
        //save session
        File f_output_session = new File(f_output_shot.getAbsolutePath()+File.separator+options_intern.file_suffix_session);
    ...
    public void run_igv_chip_atlas_data() throws Exception {
        ...
        File f_bed_data_input_root = new File(options_intern.com2pose_working_directory+File.separator+options_intern.folder_out_chip_atlas+File.separator+options_intern.folder_out_chip_atlas_peak_files);
        File f_target_genes_input_root = new File(options_intern.com2pose_working_directory+File.separator+options_intern.folder_out_target_genes_dcg);
        File f_important_tfs_per_hm = new File(options_intern.com2pose_working_directory+File.separator+options_intern.folder_out_distribution+File.separator+options_intern.folder_out_distribution_stats+File.separator+options_intern.folder_out_distribution_stats_HM);
        ...
        for(String key_tf : ordered_tfs_dcg)
        {
            ...
            for(File file_dir_tp : f_target_genes_input_root.listFiles())
            {
                ...
                {
                    File f_output_igv_shots_tp = new File(f_output_igv_shots.getAbsolutePath()+File.separator+file_dir_tp.getName());
                    ...
                    for(String key_hm : hm_to_important_tfs.keySet())
                    {
                        if(hm_to_important_tfs.get(key_hm).contains(key_tf))
                        {
                            ...
                            File f_input_target_genes_file = new File(file_dir_tp.getAbsolutePath()+File.separator+key_hm+File.separator+key_tf+File.separator+key_tf+".csv");
                            ...
                            //save session
                            File f_session = new File(f_output_igv_shots.getAbsolutePath()+File.separator+options_intern.file_suffix_session);
    ...
    public void get_chip_atlas_data() throws Exception {
        ...
        for(int i = 0; i < ordered_tfs_dcg.size(); i++)
        {
            ...
            if(found_tf)
            {
                ...
                File f_download_tf = new File(f_output_peak_files.getAbsolutePath()+File.separator+rank+"_"+ordered_tfs_dcg.get(i));
                f_download_tf.mkdir();
    ...
    public void get_top_k_target_genes_dcg() throws IOException {
        logger.logLine("[EVALUATION-PREPROCESSING] retrieve top " + options_intern.plot_top_k_genes + " target genes foreach discounted cumulative gain TF");

        File f_output_results_root = new File(options_intern.com2pose_working_directory+File.separator+options_intern.folder_out_target_genes_dcg);
        f_output_results_root.mkdir();

        File f_input_affinity_values_root = new File(options_intern.com2pose_working_directory+File.separator+options_intern.folder_name_tepic_postprocessing+File.separator+options_intern.folder_name_tepic_postprocessing_input);
        ...
        for(File file_dir_tp : f_input_affinity_values_root.listFiles())
        {
            if(file_dir_tp.isDirectory())
            {
                File f_output_tp = new File(f_output_results_root.getAbsolutePath()+File.separator+file_dir_tp.getName());
                f_output_tp.mkdir();
                ...
                for(File file_dir_tp_hm : file_dir_tp.listFiles())
                {
                    ...
                    {
                        File f_output_tp_hm = new File(f_output_tp.getAbsolutePath()+File.separator+file_dir_tp_hm.getName());
                                ...
                                while((line=br.readLine())!=null)
                                {
                                    ...
                                    for(int i =1; i < split.length;i++)
                                    {
                                        if(index_to_tf.containsKey(i))
                                        {
                                            ...
                                            {
                                                dcg.target_gene_affinity_values.get(gene_name).affinity_values.add(Double.parseDouble(split[i]));
                                            }
                                            else
                                            {
                        ...
                        for(String key_tf : tf_to_target_genes.keySet())
                        {
                            ...
                            f_output_tf.mkdir();
                            File f_output_tf_file = new File(f_output_tf.getAbsolutePath()+File.separator+key_tf+".csv");
    ...
    public void create_overview_html_report_distribution() throws Exception
    {
        ...
        File f_website_distr_analysis_html_folder_ALL_FILE = new File(f_website_distr_analysis_html_folder_ALL.getAbsolutePath()+File.separator+options_intern.html_report_home_regression_distribution_analysis_all);
    ...
    public void create_overview_html_report_before_distribution_analysis() throws Exception {
        ...
        for(int i_dont_know = 0; i_dont_know < 2; i_dont_know++)
        {
            ...
            for(File fileDir: threshold_folders)
            {
                ...
                sb_threshold.append(write_regression_coeffecient_analysis_found_table_html(Double.parseDouble(fileDir.getName()),options_intern.html_report_levels_3_steps));
    ...
    public void mix_mutually_exclusive_peaks() throws IOException {
        ...
        options_intern.tepic_input_prev = options_intern.tepic_input_directory;
        File f_annotation_check = new File(options_intern.com2pose_working_directory+File.separator+options_intern.folder_name_mix_option+File.separator+options_intern.folder_name_mix_option_preprocessing_check_chr);
    ...
    public void check_chromosomes() throws IOException {
        ...
        File chr_annotation_output = new File(root_mix_working_dir.getAbsolutePath()+File.separator+options_intern.folder_name_mix_option_preprocessing_check_chr);
                ...
                for(File fileDirHM : fileDir.listFiles())
                {
                    if (fileDirHM.isDirectory())
                    {
                        ...
                        for(File fileDirHM_sample : fileDirHM.listFiles())
                        {
                            if(fileDirHM_sample.isFile())
                            {
                                BufferedWriter bw = new BufferedWriter(new FileWriter(new File(file_output_tp_hm.getAbsolutePath()+File.separator+fileDirHM_sample.getName())));
                                BufferedReader br = new BufferedReader(new FileReader(fileDirHM_sample));
                                String line = "";
                                while((line=br.readLine())!=null)
                                {
    ...
    private String get_header_html(String level, String which_analysis) {
        ...
        if(!which_analysis.equals(options_intern.analysis_types_distribution_analysis)&&!which_analysis.equals(options_intern.analysis_types_regression_coefficient_analysis))
        {
            sb_home_front.append("  <h2 class=\"w3-center\" style='margin-left:27%'><i>"+which_analysis+"</i></h2>\n");
        }
    ...
    private void write_html_distribution_analysis_plots_hm_page(File f_website_html_hm, File fileDir_stat, String level, HashMap<String,HashMap<String,String>> tp_tf_gene_count) throws IOException {
        ...
        StringBuilder sb = new StringBuilder(get_header_html(options_intern.html_report_levels_3_steps,"Distribution Analysis: "+fileDir_stat.getName()));
