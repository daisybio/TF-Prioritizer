...
public class TPM_GC_Filter_Analysis  {
    ...
    public static void main(String[] args) throws Exception {
        ...
        HashMap<String,COM2POSE_lib> file_com2pose_obj =  find_prepare_config_files(logger, working_directory,options_intern);

        HashSet<String> interesting_tfs = get_interesting_tfs(logger,tf_list);

        File output_dir = new File(working_directory+File.separator+options_intern.tpm_gc_filter_analysis_folder_directory_name);
        output_dir.mkdir();
        File output_dir_data = new File(output_dir.getAbsolutePath()+File.separator+options_intern.tpm_gc_filter_analysis_folder_directory_name_data);
        output_dir_data.mkdir();
        File output_dir_rscripts = new File(output_dir.getAbsolutePath()+File.separator+options_intern.tpm_gc_filter_analysis_folder_directory_name_RScripts);
        output_dir_rscripts.mkdir();
        File output_dir_plots = new File(output_dir.getAbsolutePath()+File.separator+options_intern.tpm_gc_filter_analysis_folder_directory_name_plots);
    ...
    private static void create_Rscripts_execute(Logger logger, Options_intern options_intern, File output_dir_data, File output_dir_rscripts, File output_dir_plots) throws Exception {
        ...
        for(File fileDir:output_dir_data.listFiles())
        {
            if(fileDir.isDirectory())
            {
                for(File fileDir_Group : fileDir.listFiles())
                {
                    ...
                    {
                        File input = new File(fileDir_Group.getAbsolutePath()+File.separator+options_intern.tpm_gc_filter_analysis_suffix_data);
                        if(!input.exists())
                        {
                            logger.logLine("No data in " + fileDir.getName() + ": "+ fileDir_Group.getName()+"! Skipping.");
                            continue;
                        }

                        File file_output = new File(output_dir_rscripts.getAbsoluteFile()+File.separator+fileDir.getName()+File.separator+fileDir_Group.getName()+".R");

                        StringBuilder sb = new StringBuilder();
                        sb.append("library(ggplot2)\n");
                        sb.append("data <- read.csv(\"");
                        sb.append(fileDir_Group.getAbsolutePath()+File.separator+options_intern.tpm_gc_filter_analysis_suffix_data);
                        sb.append("\",sep = \"\\t\")\n");

                        sb.append("png(file=\"");
                        sb.append(output_dir_plots.getAbsolutePath()+File.separator+fileDir.getName()+File.separator+fileDir_Group.getName()+".png");
                        sb.append("\")\n");
    ...
    private static void create_data(Logger logger, Options_intern options_intern, HashSet<String> interesting_tfs, HashMap<String, COM2POSE_lib> file_com2pose_obj, File output_dir_data) throws IOException {
        logger.logLine("Create data for plotting");


        HashMap<String,HashMap<String,HashMap<String,HashMap<String,Double>>>> tf_group_hm_file_score = new HashMap<>();
        ...
        for(String s: file_com2pose_obj.keySet())
        {
            ...
            File raw_path_folder_input = new File(com2pose.options_intern.com2pose_working_directory+File.separator+options_intern.folder_out_put_DYNAMITE);
            ...
            for(File fileDir_HM : raw_path_folder_input.listFiles())
            {
                if(fileDir_HM.isDirectory())
                {
                    ...
                    for(File fileDir_HM_Group :fileDir_HM.listFiles())
                    {
                        ...
                        {
                            File input_data_dir = new File(raw_path_folder_input.getAbsolutePath()+File.separator+fileDir_HM.getName()+File.separator+fileDir_HM_Group.getName());
                            File input_data = new File(input_data_dir.getAbsolutePath()+File.separator+options_intern.file_suffix_dynamite_output_to_be_plotted);

                            for(String ss: interesting_tfs)
                            {
                                File f = new File(output_dir_data+File.separator+ss+File.separator+fileDir_HM_Group.getName());
                                f.mkdir();
                                HashMap<String,HashMap<String,HashMap<String,Double>>> x = tf_group_hm_file_score.get(ss);
                            ...
                            for(String ss: interesting_tfs)
                            {
                                tf_group_hm_file_score.get(ss.toUpperCase()).get(fileDir_HM_Group.getName()).get(com2pose.options_intern.com2pose_working_directory).put("MEAN_POS",mean_sum_pos);
                                tf_group_hm_file_score.get(ss.toUpperCase()).get(fileDir_HM_Group.getName()).get(com2pose.options_intern.com2pose_working_directory).put("MEAN_NEG",mean_sum_neg);
        ...
        for(String k_tf : tf_group_hm_file_score.keySet())
        {
            ...
            for(String k_group : group_file_hm.keySet())
            {
                ...
                File f_output = new File(output_dir_data.getAbsolutePath()+File.separator+k_tf+File.separator+k_group+File.separator+options_intern.tpm_gc_filter_analysis_suffix_data);
    ...
    private static void create_tf_folders(Logger logger, File output_dir, HashSet<String> interesting_tfs) throws IOException {
    ...
    private static HashMap<String, COM2POSE_lib> find_prepare_config_files(Logger logger, File working_directory,Options_intern options_intern) throws IOException {
        ...
        for(File fileDir : working_directory.listFiles())
        {
            ...
            if(fileDir.isDirectory() && !fileDir.getName().equals(options_intern.tpm_gc_filter_analysis_folder_directory_name))
            {
                Options_intern options_this_run = new Options_intern();
                options_this_run.config_data_path = fileDir.getAbsolutePath()+File.separator+"com2pose_template.cfg";
                options_this_run.com2pose_working_directory = fileDir.getAbsolutePath()+File.separator+options_intern.folder_name_usual_working_dir_name;
        ...
        for (String s: file_com2pose_obj.keySet())
        {
            ...
            logger.logLine("TPM: " + com2pose_obj.options_intern.tepic_tpm_cutoff+ " and GC filter: " + com2pose_obj.options_intern.deseq2_count_threshold);
        }
    ...
    private static void parseArguments(String[] args, Options_intern options_intern)
    {
        ...
        Option opt_working_dir = new Option("r","root-run-directories",true,"[REQ]: root-run-directories where all runs are in different folder including a working_dir folder in each folder");
