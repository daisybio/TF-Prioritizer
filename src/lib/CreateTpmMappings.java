...
public class CreateTpmMappings extends ExecutableStep
{
    ...
    @Override protected void execute()
    {
        ...
        if (TFPRIO.configs.general.calculateTpmLengthsEnabled.get())
        {
            ...
            try
            {
                List<String> geneIDs = readLines(f_geneIDs);
                geneIDs.remove(0);
                ...
                for (int splitIndex = 0; splitIndex < splitNumber; splitIndex++)
                {
                    ...
                    executorService.submit(() ->
                    {
                        ...
                        try
                        {
                            r_length = query("mmusculus_gene_ensembl", selectedIDs, lengthCols);
                            r_gc = query("mmusculus_gene_ensembl", selectedIDs, gcCols);
                ...
                makeSureFileExists(f_lengths);
                try (BufferedWriter writer = new BufferedWriter(new FileWriter(f_lengths)))
                {
        ...
        {
            logger.info("Get gene_lengths script is not executed, as it was deactivated by command line flag: -a");
        }
