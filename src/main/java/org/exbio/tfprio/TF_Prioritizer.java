    ...
    protected void buildFlow() {
        ...
        if (configs.chipAtlas.enabled.isSet() && configs.chipAtlas.enabled.get()) {
            ...
            */
            ...
            // remove enhancers and promoters from background data and filter for existing enhancers and promoters
            SplitLearningData splitLearningData = add(new SplitLearningData(configs,
            ...
            CombinePredictions combinePredictions = add(new CombinePredictions(configs, applyModel.outputDir));
            // Add statistics
            Statistics statistics = add(new Statistics(configs, combinePredictions.outputFiles,
                    collapsePeakFiles.outputDir, enhancerAtlas.outputFile, epdNew.outputFile));
        }
        /*
