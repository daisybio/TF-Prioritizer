...
public class Create_environments {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String default_conifg= "/home/markus/data/COM2POSE/01C_TCGA/01_TGCT/default_run_inc_tgene/com2pose_template.cfg";
        String default_command="/home/markus/data/COM2POSE/01C_TCGA/01_TGCT/default_run_inc_tgene/commandline.txt";

        create_and_copy_config_and_commandline(root_file_input, default_command, default_conifg, root_file_working_directories);
    }

    private static void create_and_copy_config_and_commandline(String root_file,String default_command, String default_conifg, String root_file_working_directories) throws IOException {
        ...
        for(File f_file:f_root.listFiles())
        {
            if(f_file.isDirectory())
            {
                ...
                File f_dir_to_atac_seq_data = new File(f_file.getAbsoluteFile()+File.separator+atac_seq+File.separator+atac_seq+File.separator+sorted_data);
                ...
                File f_dir_rna_seq_data = new File(f_file.getAbsolutePath()+File.separator+rna_seq+File.separator+formatted);
                ...
                File f_dir_rna_seq_data_geneids = new File(f_dir_rna_seq_data.getAbsolutePath()+File.separator+"names_formatted.txt");
                ...
                if(f_dir_to_atac_seq_data.listFiles().length>1)
                {
                    ...
                    File f_default_inc_tgene= new File(f_head.getAbsolutePath()+File.separator+name_default_inc_tgene);
                    ...
                    File f_default_inc_tgene_working_dir= new File(f_default_inc_tgene.getAbsolutePath()+File.separator+"working_dir");
                    f_default_inc_tgene_working_dir.mkdir();

                    File f_default_config_file = new File(f_default.getAbsolutePath()+File.separator+f_default_config.getName());
                    File f_default_config_file_inc_tgene = new File(f_default_inc_tgene.getAbsolutePath()+File.separator+f_default_config.getName());
                    ...
                    File f_default_command_txt = new File(f_default.getAbsolutePath()+File.separator+"commandline.txt");
                    ...
                    File f_default_command_txt_inctgene = new File(f_default_inc_tgene.getAbsolutePath()+File.separator+"commandline.txt");
                    BufferedWriter bw_default_command_inctgene = new BufferedWriter(new FileWriter(f_default_command_txt_inctgene));
