...
public class MANN_WHITNEYU_PLOTS_FILES {
    ...
    public void write_and_execute_scripts() throws IOException {
        ...
        script.append(
                "background <- read.csv(\"" + background.getAbsolutePath() + "\",sep=\"\\t\",comment.char = \"#\")\n");
        ...
        for (File tf : tf_data.listFiles()) {
            ...
            script.append(tf_name + "_distribution<-read.csv(\"" + tf.getAbsolutePath() +
                    "\", sep=\"\\t\", comment.char = \"#\")\n");
            script.append(tf_name + "_distribution$POPULATION<-rep(\"" + tf_name + "\",nrow(" + tf_name +
                    "_distribution))\n");
            script.append(tf_name + "_distribution_cumm <- rbind(" + tf_name + "_distribution,background)\n");
            script.append(tf_name + "_mu <- ddply(" + tf_name +
                    "_distribution_cumm, \"POPULATION\", summarise, grp.mean=mean(TF_TG_SCORE))\n");
            script.append(tf_name + "_p <- ggplot(" + tf_name +
                    "_distribution_cumm, aes(x=TF_TG_SCORE, color = POPULATION, fill=POPULATION,)) + \n" +
                    "  geom_histogram( position=\"dodge\",binwidth=1000,alpha=0.4) +\n" + "  geom_vline(data=" +
                    tf_name + "_mu, aes(xintercept=grp.mean, color=POPULATION),\n" +
                    "             linetype=\"dashed\", size = 1.5) +\n" +
                    "  labs(title = \"Distribution of TF-TG-Scores for background and " + tf_name + "\\n\")\n");
            script.append(
                    tf_name + "_p + theme(panel.background = element_rect(fill = \"#F7F7F7\", colour = \"#D0D0D0\",\n" +
                            "                                          size = 2, linetype = \"solid\"),\n" +
                            "          panel.grid.major = element_line(size = 0.5, linetype = 'solid',\n" +
                            "                                          colour = \"#D0D0D0\"), \n" +
                            "          panel.grid.minor = element_line(size = 0.25, linetype = 'solid',\n" +
                            "                                          colour = \"#D0D0D0\"),\n" +
                            "          axis.text.x = element_text(angle=30, hjust=1)) + xlim(-1,200000) + ylim(0,15000) + xlab(\"TF-TG Score\") + scale_fill_discrete(name = \"Population\", labels = c(\"Background\", \"" +
                            tf_name +
                            "\")) + scale_color_discrete(name = \"Population\", labels = c(\"Background\", \"" +
                            tf_name + "\"))");
            script.append("\n");

            script.append(
                    "\n" + tf_name + "_scores <- " + tf_name + "_distribution %>% select(TF_TG_SCORE)\n" + tf_name +
                            "_quantiles <- quantile(" + tf_name + "_scores$TF_TG_SCORE, probs = c(.25,.75))\n" +
                            tf_name + "_iqr <-(" + tf_name + "_scores$TF_TG_SCORE)\n" + tf_name + "_up <- " + tf_name +
                            "_quantiles[2]+1.5*" + tf_name + "_iqr\n" + tf_name + "_down <- " + tf_name +
                            "_quantiles[1]-1.5*" + tf_name + "_iqr\n");

            script.append("\n" + tf_name + "_ecdf <- ecdf(" + tf_name + "_scores$TF_TG_SCORE)\n");

            script.append("plot(" + tf_name +
                    "_scores, col = \"#F8766D\", main = \"Distribution of scores from PPI and random network populations\", xlab = \"score\", ylab = \"probability\")\n" +
        ...
        File f_script = new File(f_output_script.getAbsolutePath() + File.separator +
                options_intern.file_suffix_distribution_analysis_mann_whitneyU_plot_scripts);
